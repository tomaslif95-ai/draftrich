...
/* Colores de camiseta de cada equipo (no son escudos/logos, son solo
   los colores principales — evita problemas de marca registrada) */
const TEAM_COLORS = {
  "Arsenal": { bg: "#EF3340", text: "#ffffff" },
  "Manchester City": { bg: "#6CADDF", text: "#1c1c1c" },
  "Aston Villa": { bg: "#670E36", text: "#ffffff" },
  "Chelsea": { bg: "#034694", text: "#ffffff" },
  "Manchester United": { bg: "#DA291C", text: "#ffffff" },
  "Liverpool": { bg: "#C8102E", text: "#ffffff" },
  "Sunderland": { bg: "#EB172B", text: "#ffffff" },
  "Crystal Palace": { bg: "#1B458F", text: "#ffffff" },
  "Brighton": { bg: "#0057B8", text: "#ffffff" },
  "Everton": { bg: "#003399", text: "#ffffff" },
  "Newcastle": { bg: "#241F20", text: "#ffffff" },
  "Brentford": { bg: "#E30613", text: "#ffffff" },
  "Fulham": { bg: "#000000", text: "#ffffff" },
  "Tottenham": { bg: "#132257", text: "#ffffff" },
  "Bournemouth": { bg: "#DA291C", text: "#ffffff" },
  "Leeds": { bg: "#1D428A", text: "#ffffff" },
  "Nottingham Forest": { bg: "#DD0000", text: "#ffffff" },
  "Ipswich": { bg: "#0044A9", text: "#ffffff" },
  "Hull": { bg: "#F5A31A", text: "#1c1c1c" },
  "Coventry": { bg: "#78D0F7", text: "#1c1c1c" },
};
...
export default function App() {
  ...
  const [rankings, setRankings] = useState({}); // { [id]: number } — compartido entre todos, vía Supabase
    ...
    async function loadInitial() {
      ...
      if (statusErr) {
        ...
        statusRows.forEach((r) => {
          map[r.player_id] = { status: r.status, injured: !!r.injured, hidden: !!r.hidden };
        });
        setStatusMap(map);
      }
      ...
      const { data: rankRows } = await supabase.from("rankings").select("*");
      if (rankRows && isMounted) {
        const map = {};
        rankRows.forEach((r) => {
          if (r.rank != null) map[r.player_id] = r.rank;
        });
        setRankings(map);
      }
    ...
    const statusChannel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "draft_status" }, (payload) => {
        setStatusMap((prev) => {
          ...
          return { ...prev, [row.player_id]: { status: row.status, injured: !!row.injured, hidden: !!row.hidden } };
        });
      })
      .subscribe();
  ...
  const setPlayerStatus = useCallback(
    async (id, status) => {
      ...
      const hidden = current.hidden || false;
      setStatusMap((prev) => ({ ...prev, [id]: { status: nextStatus, injured, hidden } }));
      const { error } = await supabase
        .from("draft_status")
        .upsert({ player_id: id, status: nextStatus, injured, hidden, updated_at: new Date().toISOString() });
  ...
  const toggleInjured = useCallback(
    async (id) => {
      ...
      const hidden = current.hidden || false;
      setStatusMap((prev) => ({ ...prev, [id]: { status: current.status || null, injured: nextInjured, hidden } }));
      const { error } = await supabase
        .from("draft_status")
        .upsert({ player_id: id, status: current.status || null, injured: nextInjured, hidden, updated_at: new Date().toISOString() });
      if (error) setSyncNote("No se pudo sincronizar el último cambio. Revisá tu conexión.");
    },
    [statusMap]
  );
  ...
  const toggleHidden = useCallback(
    async (id) => {
      ...
      const injured = current.injured || false;
      setStatusMap((prev) => ({ ...prev, [id]: { status: current.status || null, injured, hidden: nextHidden } }));
      const { error } = await supabase
        .from("draft_status")
        .upsert({ player_id: id, status: current.status || null, injured, hidden: nextHidden, updated_at: new Date().toISOString() });
      if (error) setSyncNote("No se pudo sincronizar el último cambio. Revisá tu conexión.");
    },
    [statusMap]
  );

  const setRanking = useCallback(async (id, value) => {
    const rank = value === "" || value === null ? null : Number(value);
    setRankings((prev) => {
      const next = { ...prev };
      if (rank == null) {
        delete next[id];
    ...
    const { error } = await supabase
      .from("rankings")
      .upsert({ player_id: id, rank, updated_at: new Date().toISOString() });
    if (error) setSyncNote("No se pudo sincronizar el ranking. Revisá tu conexión.");
  }, []);
  ...
  /* ---------- Derivados ---------- */
  // Jugadores eliminados (ocultos) no se muestran en ninguna vista normal
  const visiblePlayers = useMemo(
    () => allPlayers.filter((p) => !statusMap[p.id]?.hidden),
    [allPlayers, statusMap]
  );

  const hiddenPlayers = useMemo(
    () => allPlayers.filter((p) => statusMap[p.id]?.hidden).sort((a, b) => a.name.localeCompare(b.name, "es")),
    [allPlayers, statusMap]
  );

  const counts = useMemo(() => {
    const c = { ALL: visiblePlayers.length, ARQ: 0, DEF: 0, MED: 0, DEL: 0 };
    visiblePlayers.forEach((p) => (c[p.pos] = (c[p.pos] || 0) + 1));
    return c;
  }, [visiblePlayers]);
  ...
  const teamsForMatrix = useMemo(() => {
    return TEAM_NAMES.map((teamName) => {
      const players = visiblePlayers.filter((p) => p.team === teamName && matchesFilters(p));
  ...
  }, [visiblePlayers, posFilter, searchLower, rankings]);

  const myTeam = useMemo(() => {
    const mine = visiblePlayers.filter((p) => statusMap[p.id]?.status === "mine");
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, fontFamily: "'Inter', sans-serif", color: "#2a2a24" }}>
      ...
      {showHiddenModal && (
        <HiddenPlayersModal onClose={() => setShowHiddenModal(false)} players={hiddenPlayers} onRestore={toggleHidden} />
      )}
    </div>
  );
}
...
/* ============================================================
   HEADER
   ============================================================ */
function Header({ total, loaded, view, setView, myCount, hiddenCount, onAdd, onShowHidden }) {
  return (
...
/* ============================================================
   MATRIX VIEW
   ============================================================ */
function Matrix({ teams, statusMap, rankings, setPlayerStatus, toggleInjured, toggleHidden, setRanking }) {
  if (teams.length === 0) {
    return (
...
function TeamColumn({ team, statusMap, rankings, setPlayerStatus, toggleInjured, toggleHidden, setRanking }) {
  const tc = TEAM_COLORS[team.teamName] || { bg: "#20201c", text: "#f5f2e8" };
  return (
    <div style={{ width: 232, flex: "0 0 232px", background: "#fff", borderRadius: 14, border: "1px solid #e2ddc9", overflow: "hidden" }}>
      <div style={{ background: tc.bg, color: tc.text, padding: "10px 12px" }}>
...
function PlayerRow({ player, status, ranking, onStatus, onInjured, onDelete, onRank }) {
  ...
  const handleDelete = () => {
    if (window.confirm(`¿Eliminar a ${player.name}? Podés restaurarlo después desde "Eliminados".`)) {
      onDelete();
    }
  };
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 4 }}>
        ...
        >
          <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 90 }}>{player.name}</span>
          <SpDots sp={player.sp} />
...
/* ============================================================
   ELIMINADOS (para deshacer)
   ============================================================ */
function HiddenPlayersModal({ onClose, players, onRestore }) {
  return (
    <div
      style={{
        position: "fixed",
    ...
    >
      <div
        onClick={(e) => e.stopPropagation()}
        style={{ background: "#fff", borderRadius: 14, padding: 22, width: "100%", maxWidth: 420, maxHeight: "80vh", overflowY: "auto" }}
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <h2 className="bebas" style={{ margin: 0, fontSize: 24 }}>ELIMINADOS</h2>
          <button
            onClick={onClose}
            style={{ background: "#f7f5ee", border: "1px solid #d8d2bd", borderRadius: 8, padding: "6px 10px", fontWeight: 700 }}
          >
            Cerrar
          </button>
        </div>
        ...
        {players.length === 0 ? (
          <p style={{ fontSize: 14, color: "#7a7666" }}>No eliminaste ningún jugador todavía.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: "#7a7666", display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                      <span>{p.team}</span>
                      <span style={{ background: c.bg, color: c.text, borderRadius: 5, padding: "1px 6px", fontWeight: 700 }}>{p.pos}</span>
                    </div>
                  </div>
                  <button
